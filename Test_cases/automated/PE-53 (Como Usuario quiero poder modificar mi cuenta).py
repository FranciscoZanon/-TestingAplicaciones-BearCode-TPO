# Generated by Selenium IDE
import pytest
import time
import json
from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.support import expected_conditions
from selenium.webdriver.support.wait import WebDriverWait
from selenium.webdriver.common.keys import Keys
from selenium.webdriver.common.desired_capabilities import DesiredCapabilities

class TestPrueba():
  def setup_method(self, method):
    self.driver = webdriver.Chrome()
    self.vars = {}
  
  def teardown_method(self, method):
    self.driver.quit()
  
  def test_prueba(self):
    self.driver.get("http://127.0.0.1:5000/")
    self.driver.set_window_size(974, 1040)
    self.driver.find_element(By.LINK_TEXT, "Sign In").click()
    self.driver.find_element(By.NAME, "email").click()
    self.driver.find_element(By.NAME, "email").send_keys("sample@example.com")
    self.driver.find_element(By.NAME, "password").click()
    self.driver.find_element(By.NAME, "password").send_keys("sample")
    self.driver.find_element(By.CSS_SELECTOR, "p:nth-child(3) > input").click()
    #Assert user name "Sample"
    user_name = self.driver.find_element(By.CLASS_NAME, "dropbtn").text
    assert "Sample" in user_name

    self.driver.find_element(By.CSS_SELECTOR, ".dropbtn").click()
    self.driver.find_element(By.LINK_TEXT, "Your profile").click()
    self.driver.find_element(By.LINK_TEXT, "Edit Profile").click()
    self.driver.find_element(By.NAME, "firstName").click()
    #Borramos "Sample" del cuadro de texto
    search_key = self.driver.find_element(By.NAME, "firstName")
    search_key.clear()

    self.driver.find_element(By.NAME, "firstName").send_keys("Pedro")
    self.driver.find_element(By.CSS_SELECTOR, "input:nth-child(11)").click()
    #Assert user name "Pedro" Verificación 1
    user_name = self.driver.find_element(By.CLASS_NAME, "dropbtn").text
    assert "Pedro" in user_name

    self.driver.find_element(By.ID, "logo").click()
    #Assert user name "Pedro" Verificación 2
    user_name = self.driver.find_element(By.CLASS_NAME, "dropbtn").text
    assert "Pedro" in user_name